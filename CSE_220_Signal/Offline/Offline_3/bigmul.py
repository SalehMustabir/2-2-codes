...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    if not text:
        return sign, np.array([0], dtype=np.int64)  # Handle zero case
    limbs = []
    n = len(text)
    for i in range(n, 0, -base_digits):
        start = max(0, i - base_digits)
        limb = int(text[start:i])
        limbs.append(limb)
...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    out_chunks = []
    for limb in limbs:
        total = int(limb) + carry
        carry = total // base
        out_chunks.append(str(total % base).zfill(base_digits))

    while carry > 0:
        out_chunks.append(str(carry % base).zfill(base_digits))
        carry //= base
    result = ''.join(reversed(out_chunks)).lstrip('0') or '0'
    if not result:
        return "0"
...
def multiply_transform(a, b, engine):
    ...
    """
    # TODO: implement this function
    req_len = len(a) + len(b) - 1
    if engine.name == "fft":
        N = next_power_of_two(req_len)
    else:
        N = req_len
    ...
    A = engine.transform(pad_a)
    B = engine.transform(pad_b)
    conv_freq = A * B
    conv_time = engine.inverse(conv_freq)

    result = np.round(np.real(conv_time)).astype(np.int64)
    return result, N
...
def multiply(text_a, text_b, method):
    ...
    if method == "schoolbook":
        result_limbs, N = multiply_schoolbook(limbs_a, limbs_b)
    else:
        if method == "dft":
            engine = DFTAnalyzer()
        elif method == "fft":
        ...
        result_limbs, N = multiply_transform(limbs_a, limbs_b, engine)

    product_string = from_limbs(final_sign, result_limbs, BASE_DIGITS)
    return product_string, N, limbs_a, limbs_b
...
def run_single(path, method, out_dir):
    ...
    verdict = "MATCH" if int(product_text) == expected else "MISMATCH"

    write_text(os.path.join(out_dir, "product.txt"), product_text)

    digits_a = len(text_a) - (1 if text_a[0] in "+-" else 0)
    digits_b = len(text_b) - (1 if text_b[0] in "+-" else 0)
    digits_product = len(product_text) - (1 if product_text.startswith("-") else 0)
    ...
    write_report(os.path.join(out_dir, "report.txt"), [
        ...
        "digits of product   : %d" % digits_product,
        "verification        : %s" % verdict,
    ])

    print("%s [%s]: %s" % (path, method, verdict))
    if verdict == "MISMATCH":
        print("  !! MISMATCH -- your product does not match int(a)*int(b)",
              file=sys.stderr)
