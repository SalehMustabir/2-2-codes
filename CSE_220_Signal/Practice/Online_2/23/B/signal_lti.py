...
class DiscreteSignal:
    ...
    def set_value_at_time(self, t, value):
        if t < self.start_time or t > self.end_time:
            raise ValueError("Time index is outside the signal range")
    ...
    def add(self, other):
        ...
        result_signal = DiscreteSignal(new_start, new_end)
        for t in result_signal.times():
            summed_val = self.get_value_at_time(t) + other.get_value_at_time(t)
            result_signal.set_value_at_time(t, summed_val)
    ...
    def multiply(self, scalar):
        ...
        result_signal = DiscreteSignal(new_start, new_end)
        for t in result_signal.times():
            result_signal.set_value_at_time(t, self.get_value_at_time(t) * scalar)
        return result_signal
...
class LTISystem:
    ...
    def get_response_components(self, input_signal):
        ...
        for k, x_k in input_signal.nonzero_samples():
            ...
            component_signal = shifted_h.multiply(x_k)
    ...
    def output_by_superposition(self, input_signal):
        ...
        output_signal = DiscreteSignal(output_start, output_end)
        for k, component_signal in self.get_response_components(input_signal):
            output_signal = output_signal.add(component_signal)
    ...
    def output_at_time(self, input_signal, n):
        ...
        return sum(product for _, _, _, product in contributions)
    ...
    def output(self, input_signal):
        ...
        for n in output_signal.times():
            ...
            output_signal.set_value_at_time(n, y_n)
        return output_signal
