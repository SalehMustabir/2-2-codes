...
def unit_phase(spectrum, epsilon=1e-12):
    ...
    # result[reliable] =
    # return
    result = np.ones_like(spectrum, dtype=np.complex128) # Create result with neutral phase 1+0j
    # reliable = magnitude > epsilon # Create a Boolean mask for positions where magnitude > epsilon  
    # result[reliable] = spectrum[reliable] / magnitude[reliable] # At those masked positions, divide spectrum by magnitude   
...
def swap_images(image_a, image_b, engine):
    ...
    if image_a.ndim == 3 and image_a.shape[2] == 3:
        ...
        for channel in range(3):
            ...
            plane_a = image_a[:, :, channel]
            plane_b = image_b[:, :, channel]

            result_ab, result_ba = swap_plane_spectra(
                plane_a,
                plane_b,
                engine
            )

            result_ab_planes.append(result_ab)
            result_ba_planes.append(result_ba)

        result_ab = np.stack(result_ab_planes, axis=2)
        result_ba = np.stack(result_ba_planes, axis=2)
